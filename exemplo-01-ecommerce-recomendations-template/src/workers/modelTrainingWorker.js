...
// ? Transform values between 0 and 1 (normalização) para ajudar o modelo a aprender melhor
...
function makeContext(catalog, users) {
  ...
  users.forEach((user) => {
    user.purchases.forEach((p) => {
      ...
      ageCounts[p.name] = (ageCounts[p.name] || 0) + user.age;
    });
  });

  const producAvgAgeNorm = Object.fromEntries(
    catalog.map((product) => {
